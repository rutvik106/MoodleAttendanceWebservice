...
// eg: if you use "yourhostname.com/moodle35" to open your modle use define('MOODLE_DIR', '/moodle35');
	...
	if (!$result) {
		...
		if (mysqli_num_rows($result) > 0) {
			while ($row = mysqli_fetch_array($result)) {
				$count++;
				$data = array('no' => $count, 'name' => $row[0]);
				$post_data[] = $data;
...
function login($conn, $userName, $password)
{
	...
	$url = "http://" . $CFG->dbhost  . "" . MOODLE_DIR . "/webservice/rest/server.php?wstoken=$token&wsfunction=core_webservice_get_site_info";
	$data = file_get_contents($url);
	//$result = json_decode($data, true);
